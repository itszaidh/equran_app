...
class Crinkle with Diagnosticable {
  ...
  })  : min = min ?? -2.0 * intensity,
        max = max ?? 2.0 * intensity,
        scalar = scalar ??
            (isDirectional
                ? const Scalar(horizontal: 1.0, vertical: 0.0)
                : Scalar.identity),
        period = period ??
            (speed == 0.2
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties
      ...
      ..add(DoubleProperty('intensity', intensity, defaultValue: 1.0))
      ..add(DiagnosticsProperty<bool>('isDirectional', isDirectional,
          defaultValue: true));
  }
}
